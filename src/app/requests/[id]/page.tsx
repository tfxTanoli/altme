...
export default function RequestDetailPage({ params }: { params: { id: string } }) {
  ...
  React.useEffect(() => {
    ...
    const unsubRequest = onSnapshot(requestDocRef, async (docSnap) => {
        ...
        const ownerDoc = await getDoc(doc(firestore, 'users', requestData.userId));
        if (ownerDoc.exists()) {
          setRequestOwner({ id: ownerDoc.id, ...ownerDoc.data() } as User);
        }
        ...
        if (fetchedReviews.length > 0) {
            ...
            const reviewersQuery = query(collection(firestore, 'users'), where('__name__', 'in', reviewerIds));
            const reviewersSnap = await getDocs(reviewersQuery);
            const reviewersMap = new Map(reviewersSnap.docs.map(d => [d.id, { id: d.id, ...d.data() } as User]));
    ...
    const unsubBids = onSnapshot(bidsQuery, async (bidsSnapshot) => {
        ...
        if (fetchedBids.length > 0) {
            ...
            const usersQuery = query(collection(firestore, 'users'), where('__name__', 'in', bidderIds));
            const usersSnapshot = await getDocs(usersQuery);
            const usersMap = new Map(usersSnapshot.docs.map(d => [d.id, { id: d.id, ...d.data() } as User]));
  ...
  const handleApproveDelivery = async () => {
      ...
      const paymentAmount = request.acceptedBidAmount;
  ...
  const handleCompleteReview = async (rating: number, comment: string) => {
      ...
      try {
          ...
          batch.update(reviewerRef, { pendingReviewCount: increment(-1) });
            ...
            {chatRoom && (isOwner || isHiredPhotographer) && (
                 <Card>
                    ...
                    <CardContent className="h-[600px]">
                        <ChatView 
                            partner={isOwner ? (bids.find(b => b.userId === request.hiredPhotographerId)?.bidderUser) : requestOwner}
                            chatRoom={chatRoom}
                            allUsersMap={new Map([...bids.map(b => [b.userId, b.bidderUser] as [string, User]), requestOwner ? [requestOwner.id, requestOwner] : []].filter((entry): entry is [string, User] => !!entry[1]))}
                        />
                    </CardContent>
                </Card>
            )}
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        {clientReview && photographerReview ? (
                            <>
                                <ReviewCard review={clientReview} reviewer={requestOwner} />
